...
class AdminaplikasiController extends Controller {
	...
	public $module = 'adminaplikasi';
	static $per_page	= '50';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Adminaplikasi();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'adminaplikasi',
		...
		switch ($task)
		{
			default:
				...
				if ($validator->passes()) 
				{
					...
					$cek = DB::table('tb_users')->select('username')->where('username',$request->username)->pluck('username')->first();
					$arr = array(
						'group_id'=> 3,
					...
					if(isset($cek)){
						DB::table('tb_users')->where('username',$request->username)->update($arr);

					}else{
						$user = DB::table('tb_users')->insert($arr);
	...
	public static function display(  )
	{
		...
		$model  = new Adminaplikasi();
		$info = $model::makeInfo('adminaplikasi');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('adminaplikasi.public.view',$data);			
			}			
		} 
		else {
			...
			return view('adminaplikasi.public.index',$data);	
		}
