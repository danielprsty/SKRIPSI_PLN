...
class HomeController extends Controller {
	...
	public function set_active_menu(Request $request){
		session(['menu'=> $request->menu ]);
		return response()->json(['status'=>'success']);
	}

	public function scan_sku($id){
		$get= DB::table('tb_mapping_sku as f')
        ->select('a.*','b.name as site','c.room_name as room','c.floor','f.id as id_mapping')
        ->leftJoin('tb_site as b', 'f.id_site', '=', 'b.id')
        ->leftJoin('tb_room as c', 'f.id_room', '=', 'c.id')
        ->leftJoin('tb_sku as a', 'f.id_sku', '=', 'a.id')
        ->where('a.sku',$id)
        ->first();
        ...
        $data = array(
            ...
            'floor' => $get->floor,
            'id' =>$get->id,
            'id_mapping'=>$get->id_mapping
		);

		return view( 'scan_sku', $data);	
