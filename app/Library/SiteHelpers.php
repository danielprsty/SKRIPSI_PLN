...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	{
		$avatar = '<img alt="" src="http://www.gravatar.com/avatar/'.md5(session('eid')).'" class="avatar user-image" width="'.$width.'" />';
		if($id =='') {
			$id = session('uid');
		} 
		...
		if(count($Q)>=1) 
		{
			...
			if($row->avatar !='' ) 	
			{
				...
				{
					return  '<img src="'.asset('uploads/users').'/'.$row->avatar.'" border="0" width="'.$width.'" class="avatar user-image" />';
				} else {
					return $avatar;
				}	
			} else {
  ...
  static function ResumeUserStatus()
  {
	...
	public function penyebut($nilai) {
		...
		if ($nilai < 12) {
			...
			$temp = $this->penyebut($nilai/10)." puluh". $this->enyebut($nilai % 10);
		} else if ($nilai < 200) {
			...
			$temp = penyebut($nilai/1000000) . " juta" . $this->penyebut($nilai % 1000000);
	...
	public static function terbilang($angka) {
		...
		$terbilang="";
		 if ($angka < 12){
			 ...
			 $terbilang= static::terbilang($angka - 10) . " Belas";
		 }
		 else if ($angka < 100){
			 $terbilang= static::terbilang($angka / 10) . " Puluh" . static::terbilang($angka % 10);
		 }
		 else if ($angka < 200){
			 $terbilang= " seratus" . static::terbilang($angka - 100);
		 }
		 else if ($angka < 1000){
			 $terbilang= static::terbilang($angka / 100) . " Ratus" . static::terbilang($angka % 100);
		 }
		 else if ($angka < 2000){
			 $terbilang= " seribu" . static::terbilang($angka - 1000);
		 }
		 else if ($angka < 1000000){
			 $terbilang= static::terbilang($angka / 1000) . " Ribu" . static::terbilang($angka % 1000);
		 }
		 else if ($angka < 1000000000){
			$terbilang= static::terbilang($angka / 1000000) . " Juta" . static::terbilang($angka % 1000000);
		 }
			return $terbilang;
	}
