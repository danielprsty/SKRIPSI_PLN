    ...
    <div class="sidebar-compact" id="side-nav">
      <ul>
          <li class="logo" >
            ...
            <img style="margin:auto;" src="{{ asset('uploads/images/'.config('sximo')['cnf_logo'])}}" alt="{{ config('sximo')['cnf_appname'] }}" width="40" />
            @else
            <img  style="margin:auto;" src="{{ asset('uploads/logo.png')}}" alt="{{ config('sximo')['cnf_appname'] }}"  width="40" />
            @endif 
          </li> 
         
          <li>
            <a href="{{url('dashboard')}}" code="menu-home" @if( $active !='user' && $active !='core'  && $active !='sximo' && $active !='cms') class="active" @endif ><i class="lni-home"></i> Home
            </a>
          </li>  
          @foreach ($sidebar as $menu)
          <li>
            <a href="javascript:;" code="{{$menu['menu_id']}}" @if( $active !='user' && $active !='core'  && $active !='sximo' && $active !='cms') class="active" @endif ><i class="{{$menu['menu_icons']}}"></i> {{$menu['menu_name']}}
            </a>
          </li>    
          @endforeach         
          <li>
            <a href="javascript:;"  code="menu-profile" @if( $active =='user') class="active" @endif>
              <i class="lni-user"></i> User
            </a>
          </li>
           
          @if( session('gid') =='1' || session('gid') =='2' )
           <li  >
            <a href="javascript:;" code="menu-page" @if( $active =='cms' )  class="active" @endif ><i class="lni-rss-feed"></i> Artikel
            </a>
          </li>
          <li > <a href="javascript:;" code="menu-admin" @if( $active =='core') class="active" @endif ><i class="lni-control-panel"></i> User Management</a></li>
            @if( session('gid') =='1' )
            <li > <a href="javascript:;" code="menu-app"  @if( $active =='sximo') class="active" @endif><i class="lni-grid-alt"></i> Aplication</a></li>
            @endif
          @endif
          <li > 
            <a href="{{ url('notification') }}"  code="menu-home"  >
              <span class="badge badge-pill badge-danger countNotif">0</span>
              <i class="lni-alarm"></i> Notification
          </a></li>
          <li  ><a href="{{ url('user/logout')}}"class="confirmLogout" ><i class="lni-power-switch"></i> Logout</a></li>
      </ul>    
    </div>
